...
export const fetchCards = createAsyncThunk('kanban/fetchCards', async (boardId: string) => {
  const response = await fetch(`http://localhost:3000/api/cards?boardId=${boardId}`);
  if (!response.ok) {
...
export const updateCard = createAsyncThunk('kanban/updateCard', async (card: Card) => {
  const response = await fetch(`http://localhost:3000/api/cards/${card._id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(card),
  });
...
export const deleteCard = createAsyncThunk('kanban/deleteCard', async (cardId: string) => {
  const response = await fetch(`http://localhost:3000/api/cards/${cardId}`, { method: 'DELETE' });
  if (!response.ok) {
    const errorText = await response.text();
...
//   console.log('Dispatching fetchCards, making request to /api/cards');
//   const response = await fetch('http://localhost:3000/api/cards');
